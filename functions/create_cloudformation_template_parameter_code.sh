...
create_cloudformation_template_parameter_code(){
    ...
    if [ -z $RESOURCE_TYPE ]; then echo "$RESOURCE_TYPE not set in create_cloudformation_template_parameter_code" >&2; exit 1; fi
    if [ -z $SCHEMA_B64 ]; then echo "$SCHEMA_B64 not set in create_cloudformation_template_parameter_code" >&2; exit 1; fi
    if [ -z $TEMPLATE_FILE_PATH ]; then echo "$TEMPLATE_FILE_PATH not set in create_cloudformation_template_parameter_code" >&2; exit 1; fi
