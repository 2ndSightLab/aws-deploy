...
create_deploy_script_resource_code() {
    ...
    echo "Properties JSON for $RESOURCE_TYPE: $properties_json"
     ...
     while read -r property; do
            description_b64=$(echo "$properties_json" | jq -r --arg prop "$property" '.[$prop].description | (if type == "string" then fromjson? // . else . end) | tojson | @base64')
            echo "echo \"Description: $(echo "$description_b64" | base64 -d)\"" >> "$SCRIPT_FILE_PATH"
