...
create_deploy_script_for_resource() {
    ...
    if [ -z $SERVICE_NAME ]; then echo "$SERVICE_NAME not set in create_deploy_script_for_resource" >&2; exit 1; fi
    if [ -z $RESOURCE_NAME ]; then echo "$RESOURCE_NAME not set in create_deploy_script_for_resource" >&2; exit 1; fi
    if [ -z "$ENV_PROFILE" ]; then echo "$ENV_PROFILE not set in create_deploy_script_for_resource" >&2; exit 1; fi
    if [ -z "$REGION" ]; then echo "$REGION not set in create_deploy_script_for_resource" >&2; exit 1; fi
